...
const ChatPage = () => {
  ...
  const performDiabetesAnalysis = async () => {
    ...
    try {
      ...
      console.log('📋 Analysis data stringified:', JSON.stringify(analysisData, null, 2));
      console.log('📋 Current messages before update:', messages);
      
      if (analysisData && analysisData.status === 'success') {
        console.log('✅ Entering success block - analysis successful!');
        ...
        const finalResponse = cleanResponse || "Analysis completed successfully. Please check the console for detailed results.";
        ...
        setMessages((prev) => {
          ...
          if (updated.length > 0 && updated[updated.length - 1].Ai_response.includes("I'm analyzing your glucose report")) {
            updated[updated.length - 1] = { question: "", Ai_response: finalResponse };
          } else {
            updated.push({ question: "", Ai_response: finalResponse });
          }
          console.log('📝 Messages updated after analysis:', updated);
          return updated;
        });
      ...
      setMessages((prev) => {
        ...
        if (updated.length > 0 && updated[updated.length - 1].Ai_response.includes("I'm analyzing your glucose report")) {
          updated[updated.length - 1] = { 
            question: "", 
            Ai_response: `I apologize, but I encountered an error while analyzing your report. Error: ${error.message || 'Unknown error'}. Please try again.` 
          };
        } else {
          updated.push({ 
            question: "", 
        ...
        console.log('📝 Error message added to chat:', updated[updated.length - 1]);
        return updated;
      });
      await fallbackChatAnalysis();
    } finally {
      ...
      await startConversation(
        ...
        (chunk) => {
          ...
          setMessages((prev) => {
            ...
            // Create a new last message object to avoid mutation
            if (last.question === "") {
              updated[lastIndex] = { 
                ...last, 
                Ai_response: last.Ai_response + text 
