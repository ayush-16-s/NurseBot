...
const PdfManager = () => {
  ...
  const handleViewInModal = async (file) => {
    try {
      ...
      console.log('Debug: File _id:', file._id, 'type:', typeof file._id);
      
      // Handle MongoDB ObjectId format - _id might be { $oid: "..." } or a string
      let fileId = null;
      if (file._id) {
        if (typeof file._id === 'object' && file._id.$oid) {
          fileId = file._id.$oid;
        } else if (typeof file._id === 'string') {
          fileId = file._id;
        } else if (file._id.toString) {
